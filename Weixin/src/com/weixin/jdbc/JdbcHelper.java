...
public class JdbcHelper {
     ...
     * @throws SQLException
     */
    public static List<User> queryUser(String sql) throws SQLException { 	
        ...
        try {
            ...
            while(rs.next())
            {
            User user=new User(rs.getInt("Id"),rs.getString("Openid"), rs.getString("Name"),rs.getString("Teacherid"),rs.getString("Sex"),rs.getDate("birthday"),rs.getString("Tel"),rs.getString("Protitle"),rs.getString("College"));
            users.add(user);
            }
            return users;
        } catch (SQLException e) {
    ...
    public static List<User> queryUser(String sql,String...paramters) throws SQLException { 	
    	List<User> users =new ArrayList<User>();
        ResultSet rs = null;
        try {
            getPreparedStatement(sql);
            for (int i = 0; i < paramters.length; i++) {
            	 preparedStatement.setString(i+1,paramters[i]+"%");//自动添加单引号 （包装后的参数）    
            }
            rs = preparedStatement.executeQuery();
            while(rs.next())
            {
